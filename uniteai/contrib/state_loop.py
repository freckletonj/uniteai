...
def parse_state_and_response(string):
    state_pattern = r"START_STATE:\n(.*?)\nEND_STATE:"
    response_pattern = r"START_RESPONSE:\n(.*?)\nEND_RESPONSE:"

    state = re.search(state_pattern, string, re.DOTALL)
    response = re.search(response_pattern, string, re.DOTALL)
...
STATE_START_TAG = ':STATE_START_TAG:'
STATE_END_TAG = ':STATE_END_TAG:'
NAME = 'state_loop'
log = mk_logger(NAME, logging.DEBUG)

class StateLoopActor(Actor):
    def __init__(self):
...
def openai_autocomplete(engine, text, max_length):
    ''' Stream responses from StateLoop's API as a generator. '''
...
def initialize(config, server):
    ...
    # Actor
    server.add_actor(NAME, StateLoopActor)
